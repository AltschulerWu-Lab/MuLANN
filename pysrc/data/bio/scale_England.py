...
from pysrc.utils.settings import Settings
from pysrc.data.stitch_England import CaieStitcher
from pysrc.utils import bgs_folder, image_setting_file, output_image_name, england_plates
from pysrc.utils.baseline import SingleImagePrinter


class CaieScaler(CaieStitcher):
    def __init__(self, metadata_dir_, outputfolder_, levelsets=[(0, 15000), (0, 27000), (0, 20000)], zoom=0.25, verbose=False):
        super(CaieScaler, self).__init__(metadata_dir_)

        self.zoom = zoom
        self.levelsets = levelsets
        self.verbose = verbose
        self.metadata_dir = metadata_dir_
        self.image_setting_file = os.path.join(metadata_dir_, image_setting_file)
        self.outputfolder = outputfolder_
        self.scaledfolder = os.path.join(os.path.dirname(self.outputfolder), 'ScaledCaie')

    def scaledown(self, plate_list):
    ...
    def move_images(self, plate_list, final_outputfolder_):
        for plate in plate_list:
            print(plate)
            if not os.path.isdir(os.path.join(final_outputfolder_, plate)):
                os.mkdir(os.path.join(final_outputfolder_, plate))
            ...
            for well in wells:
                ...
                if not os.path.isdir(os.path.join(final_outputfolder_, plate, drug_class)):
                    os.mkdir(os.path.join(final_outputfolder_, plate, drug_class))

                cpd = line.Drug.values[0]
                curr_finalfolder = os.path.join(final_outputfolder_, plate, drug_class, cpd)
...
if __name__ == '__main__':
    ...
    scaler = CaieScaler(metadata_dir_=metadata_dir, outputfolder_=outputfolder)
