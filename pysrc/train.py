...
#            logger.add_scalar('lr', lr, global_step)
        ...
        if (epoch % options.statinterval) == 0:
            tgt_test_loss, tgt_acc, tgt_acc_domain = test(model, target.unsup_evalset, device, options=options, flag='target')
            src_test_loss, src_acc, src_acc_domain = test(model, source.sup_evalset, device, options=options, flag='source')
...
def test(model, data_loader, device, options, flag):
    ...
    print(f"Avg {flag} Loss = {loss:.6f}, Avg Accuracy = {acc:.2%}, {acc_}/{n_total}, Avg Domain Accuracy = {acc_domain:2%}")
    model.train()
